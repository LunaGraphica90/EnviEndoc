...
const getSubstanceInMap = (urlTable, substanceCas, cnepDate) => {
  ...
  Promise.all(
    ...
    CNEP = CNEP.filter(
      (item) =>
        item.properties.cas_number.includes(substanceCas) &&
        item.properties['date de fin de prélèvement'].slice(0, 4) === cnepDate
    )
    ...
    const sitesUtilise = [
      ...new Set(CNEP.map((item) => item.properties['code site'])),
    ]
    CNEP_sites = CNEP_sites.filter((item) =>
      sitesUtilise.find((codeSite) => codeSite === item.properties['code site'])
    )
      ...
      source: new VectorSource({
        ...
        features: CNEP_sites.map((item) => {
          return new Feature({
            ...
            samples: CNEP.filter(
              (sample) =>
                sample.properties['code site'] === item.properties['code site']
            ),
          })
        }),
      }),
    ...
    const BNVD_LAYER = new VectorLayer({
      source: new VectorSource({
        features: BNVD.filter(
          (item) => typeof item.properties.value === 'number'
        ).map((item) => {
          return new Feature({
            ...item.properties,
            geometry:
              item.geometry.type === 'Polygon'
                ? new Polygon(
                    item.geometry.coordinates.map((value) =>
                      value.map((coordinates) => fromLonLat(coordinates))
                    )
                  )
                : new MultiPolygon(
                    item.geometry.coordinates.map((value) =>
      ...
      style: function (feature) {
        ...
        const findColor = colorByValue.find(
          (obj) => value >= obj.valueMin && value <= obj.valueMax
        )
        const opacity = '1'
    ...
    const baseLayerElements = document.querySelectorAll(
      '.formulaire > fieldset > div > label > input[type=checkbox]'
    )
...
formMapElt.addEventListener('submit', (e) => {
  ...
  const substanceFiltreValue =
    formMapElt.querySelector('#substance-filtre').value
  ...
  getSubstanceInMap(
    [BNVD_URL, './datas/CNEP/CNEP.json', './datas/CNEP/CNEP_site.json'],
    substanceFiltreValue,
    cnepDateValue
  )
})
...
const getInfosCNEP = (properties, parentElt) => {
  ...
  properties.samples.sort(
    (a, b) =>
      Date.parse(a.properties['date de début de prélèvement']) -
      Date.parse(b.properties['date de début de prélèvement'])
  )
  ...
  const dateBeginning = getDate(
    properties.samples[0].properties['date de début de prélèvement']
  ).slice(0, 10)
  const dateEnd = getDate(
    properties.samples[properties.samples.length - 1].properties[
  ...
  properties.samples.forEach((item, iteration) => {
    ...
    Object.keys(item.properties).forEach((key, i) => {
      if (
        key === 'date de début de prélèvement' ||
        key === 'date de fin de prélèvement'
      ) {
...
const getInfosBNVD = (properties, parentElt) => {
  ...
  tableKeyBNVD.forEach((item) => {
    ...
    if (item.key === 'value') {
      content += `<li>${item.name}: ${properties[item.key]} ${
        properties.unit
      }s </li>`
    } else if (item.key === 'subst_URL_for_deduct') {
      content += `<li>${item.name}: <a target="_blank" href='${
        properties[item.key]
      }'>DEDuCT (s'ouvre dans une nouvelle fenêtre)</a> </li>`
