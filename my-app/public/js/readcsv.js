...
const parseCSV = () => {
  ...
  const csvFile =
    lang === 'fr'
      ? './documents/csv/listeFR.csv'
      : './documents/csv/listeENG.csv'
      ...
      Object.values(csvData).forEach((rowData) => {
        ...
        if (!isEmptyRow) {
          ...
          columnNames.forEach((columnName) => {
            ...
            if (
              columnName === 'URL vers la fiche DEDuCT' ||
              columnName === 'URL to the DEDuCT page'
            ) {
              const link = document.createElement('a')
              link.href = value // Définit l'URL du lien
              ;(link.target = '_blank'), link.classList.add('external')
